...
function startGateway() {
  ...
  // Loading animation until OpenClaw starts
  const frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
  let i = 0;
  let started = false;
  ...
  const spinner = setInterval(() => {
    if (!started) {
      process.stdout.write(`\r${frames[i++ % frames.length]} Starting OpenClaw gateway (loading proot)...`);
    }
  }, 80);

  const gateway = runInProotWithCallback('openclaw gateway --verbose', () => {
    // Called when first output received
    if (!started) {
      started = true;
